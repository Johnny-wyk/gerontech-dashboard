...
with tab5:
    st.markdown("### 基于 IoT 与 AI 的资源错配解决战略 (Tech-Driven Outreach Strategy)")
    
    st.info("💡 **核心痛点重塑 (Pain Point Redefined):**\n作为非营利机构 (NGO)，我们的核心挑战并非“缺乏设备”，而是**严重的供需错配 (Mismatch between supply and demand)** 导致的**极低利用率 (Low Utilization)**。设备在仓库吃灰，而有需要的隐蔽长者却无人问津。以下是我们如何通过 IoT 数据闭环与 AI 预测来解决这一错配的战略：")
    
    col_s1, col_s2, col_s3 = st.columns(3)
    
    with col_s1:
        st.markdown("#### 1. IoT 数据采集与动态资源池")
        st.markdown("""
        **如何应用技术 (How it works)**：
        - **IoT 状态监控**：在轮椅、护理床等核心设备上加装低功耗 IoT 标签（如 NB-IoT/RFID），实时采集设备的**位置轨迹**与**使用频率**。
        - **数据价值**：一旦 IoT 监测到某台设备连续 14 天未被移动（低利用率），系统会自动触发社工探访指令，核实是否需要回收。
        
        **商业/社会效益 (Business Benefit)**：
        彻底打破过去“一租出去就失联”的盲盒状态，极大加速设备的**流转率 (Turnover Rate)**，让有限的 NGO 资源服务更多长者。
        """)
    ...
    with col_s3:
        st.markdown("#### 3. 分层精准营销 (Segmented Outreach)")
        st.markdown("""
        **如何应用技术 (How it works)**：
        基于大数据分析，我们将营销对象切割为两类人群，实行完全不同的触达策略：
        - **针对长者 (观念固执/数字盲)**：结合 Tab 2 的热力图预测，将“乐龄科技流动体验车”直接开进需求爆发前夕的高危屋邨，用“眼见为实”打破心理防线。
        - **针对子女 (真实决策者)**：通过社交媒体，对定位在红区（渗透率倒挂区）的中年人群进行靶向广告投放，核心诉求直击：“**减轻您的日常照护压力与腰肌劳损**”。
        
        **商业/社会效益 (Business Benefit)**：
        用最低的营销成本，精准撬动真正的买单人和决策者，有效消除信息差。
        """)
