...
with tab4:
    ...
    with col_s1:
        st.markdown("#### 1. 医疗与社工网络下沉 (B2B2C 模式)")
        st.markdown("""
        **数据依据**：Tab 1 的归还原因显示，极高比例的用户是因为“长者离世”或“入住安老院舍”而终止租赁。这说明我们的服务高度集中在**长者生命周期的最后阶段 (End-of-life care)**。
        
        **战略行动**：
        由于处于这一阶段的长者往往已经失去了主动搜索信息的能力，我们必须依赖专业人士的转介。
        - **重点合作**：与高缺口老区（如观塘、黄大仙）的**医院离院规划组 (Discharge Planning Team)**、职业治疗师 (OT) 及**地区康健中心 (DHC)** 建立深度合作。
        - **工具赋能**：将乐龄科技目录直接嵌入社工的探访终端，实现“床边评估、一键租赁”，截获最刚性的隐蔽需求。
        """)
        
    with col_s2:
        st.markdown("#### 2. 靶向照顾者的数字营销")
        st.markdown("""
        **数据依据**：从 Tab 1 的年龄分布可以看出，实际使用设备的长者年龄中位数极高（70-90岁）。这群人极少自主进行线上租赁，**真正的决策者和买单者是他们 40-60 岁的中年子女 (Carers)**。
        
        **战略行动**：
        - **定向投放**：在 Facebook、Google 甚至 LinkedIn 上，对 40-60 岁、IP 定位或通勤轨迹在“渗透率倒挂红区”（参考 Tab 2 双轴图）的人群进行精准广告投放。
        - **文案转换**：广告诉求从传统的“提升长者生活质量”转变为“**大幅减轻您的日常照护压力与腰肌劳损**”。主推在 Tab 1 饼图中占绝对主导的轮椅与护理床套装。
        """)
        
    with col_s3:
        st.markdown("#### 3. 动态库存调拨与流动体验")
        st.markdown("""
        **数据依据**：结合 Tab 2 的渗透率双轴对比图（部分地区老龄化率极高但实际租赁量极低）以及 Tab 3 的 Prophet 季节性需求预测。
        
        **战略行动**：
        - **O2O 社区渗透**：针对完全不会上网的隐蔽群体，在需求爆发期前 1-2 个月（基于预测模型），将“乐龄科技流动宣传车”直接开进 Tab 2 识别出的高缺口屋邨。
        - **前置仓策略**：眼见为实是打消长者顾虑的唯一方式。通过将高频刚需设备（如轮椅）提前调拨至这些地区的合作机构（前置仓），实现即看即租，打破服务可达性的最后一公里障碍。
        """)
